  ...
  for file in os.listdir(sourceDir):
    ...
    if file in verbatim:
      ...
      try:
        ...
        traceback.print_exc()
        print >>sys.stderr
      known['rules_for_ad-safe.txt'] = True
    known[file] = True
...
def processSubscriptionFile(sourceDir, targetDir, file, timeout):
  ...
  writeRule(os.path.join(targetDir, 'rules_for_ad-safe.txt'), lines)
...
!Url:http://rules.adfiltering-rules.asia/svn/trunk/lastest/rules_for_ad-safe.txt''', line)
      result.append(line)
    elif line.find('#') >= 0:
      # 如果是元素隐藏规则
...
'''import shutil
import os
if os.path.isfile('.' + 'rules_for_ad-safe.txt'):
  os.system('rm -fr rules_for_ad-safe.txt')
else:
  shutil.copy('./Temp/rules_for_ad-safe.txt', '.')'''
#把临时生成的文件移动回根目录，同时去除所有的空白行
# coding=utf-8
file1 = open("./Temp/rules_for_ad-safe.txt","r")
file2 = open("rules_for_ad-safe.txt","w")
while 1:
 text = file1.readline()
 if( text == '' ):
  break
